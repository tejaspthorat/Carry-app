...
_typesense_host = (os.getenv('TYPESENSE_HOST') or '').strip()
_typesense_port = (os.getenv('TYPESENSE_HOST_PORT') or '').strip()
_typesense_api_key = (os.getenv('TYPESENSE_API_KEY') or '').strip()

if _typesense_host and _typesense_port and _typesense_api_key:
    client = typesense.Client(
        {
            'nodes': [{'host': _typesense_host, 'port': _typesense_port, 'protocol': 'https'}],
            'api_key': _typesense_api_key,
            'connection_timeout_seconds': 2,
        }
    )
else:
    logger.warning('STARTUP: Typesense env vars missing; keyword conversation search will be disabled.')
    client = None
...
def search_conversations(
    ...
    try:
        if client is None:
            logger.warning('search_conversations skipped because Typesense is not configured')
            return {'items': [], 'total_pages': page, 'current_page': page, 'per_page': per_page}
