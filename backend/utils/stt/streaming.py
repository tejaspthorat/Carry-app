...
_deepgram_api_key = (os.getenv('DEEPGRAM_API_KEY') or '').strip()
if not _deepgram_api_key:
    logger.warning('STARTUP: DEEPGRAM_API_KEY missing; streaming transcription will fail at request time.')
    _deepgram_api_key = 'not-set'

headers = {"Authorization": f"Token {_deepgram_api_key}", "Content-Type": "audio/*"}
...
deepgram = DeepgramClient(_deepgram_api_key, deepgram_options)

# unused fn
deepgram_beta = DeepgramClient(_deepgram_api_key, deepgram_cloud_options)
