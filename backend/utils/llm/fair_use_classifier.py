...
if not _openai_api_key:
    logger.warning('STARTUP: OPENAI_API_KEY missing; fair-use classifier will fail at request time.')
    _openai_api_key = 'not-set'
_classifier_llm = ChatOpenAI(
    model=CLASSIFIER_MODEL, api_key=_openai_api_key, callbacks=[get_usage_callback()], request_timeout=120, max_retries=1
)
