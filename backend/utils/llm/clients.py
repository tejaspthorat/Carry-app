...
def _optional_api_key(env_name: str, feature_name: str) -> str:
    api_key = (os.getenv(env_name) or '').strip()
    if not api_key:
        logger.warning(
            'STARTUP: %s missing; %s will fail at request time until the env var is set.',
            env_name,
            feature_name,
        )
        return 'not-set'
...
_default_anthropic_client = anthropic.AsyncAnthropic(
    api_key=_optional_api_key('ANTHROPIC_API_KEY', 'Anthropic chat'),
    timeout=120.0,
    max_retries=1,
)
...
llm_mini = ChatOpenAI(
    model='gpt-4.1-mini',
    api_key=_optional_api_key('OPENAI_API_KEY', 'OpenAI chat'),
...
# ---------------------------------------------------------------------------
_embeddings_default = OpenAIEmbeddings(
    model="text-embedding-3-large",
    api_key=_optional_api_key('OPENAI_API_KEY', 'OpenAI embeddings'),
)
