...
from utils.opus_loader import add_bundled_opus_dll_directory

add_bundled_opus_dll_directory()
    ...
    if is_multi_channel:
        ...
        if codec == 'opus':
            if opuslib is None:
                ...
                logger.error(f"Opus codec requested but native libopus is not available {uid} {session_id}")
                await websocket.close(code=websocket_close_code, reason="Opus codec is not available")
                return
