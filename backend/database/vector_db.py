...
_pinecone_api_key = (os.getenv('PINECONE_API_KEY') or '').strip()
_pinecone_index_name = (os.getenv('PINECONE_INDEX_NAME') or '').strip()
...
if _pinecone_api_key and _pinecone_index_name:
    ...
    logger.warning(
        'STARTUP: PINECONE_API_KEY or PINECONE_INDEX_NAME missing; vector search features will fail at request time.'
    )
    index = None
