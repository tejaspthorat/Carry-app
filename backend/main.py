...
def _warn_missing_optional_env_vars():
    optional_env_vars = [
        'OPENAI_API_KEY',
        'ANTHROPIC_API_KEY',
        'DEEPGRAM_API_KEY',
    ...
    missing = [name for name in optional_env_vars if not (os.getenv(name) or '').strip()]
    if missing:
        logger.warning(
            'STARTUP: optional env vars missing; related features will fail at request time, '
            'but the API server will continue starting: %s',
            ', '.join(missing),
        )


_warn_missing_optional_env_vars()
...
app.include_router(live_transcript.router)
